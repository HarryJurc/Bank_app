...
def get_operations_data(filepath: str = "../data/operations.xlsx") -> pd.DataFrame:
    ...
    if not os.path.exists(filepath):
        logging.error(f"Файл данных {filepath} не найден.")
        raise FileNotFoundError(f"Файл данных {filepath} не найден.")

    df = pd.read_excel(filepath)
    df.columns = df.columns.str.strip()
    logging.info(f"Чтение данных выполнено, столбцы: {df.columns.tolist()}")

    df["Дата операции"] = pd.to_datetime(df["Дата операции"], errors="coerce", dayfirst=True)
    if df["Дата операции"].isnull().any():
        logging.warning(f"Некоторые даты не удалось преобразовать в 'Дата операции'.")
